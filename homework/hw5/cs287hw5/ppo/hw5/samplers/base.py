...
class Sampler(object):
    ...
    Args:
        ...
        policy (hw5.policies.policy) : policy object
...
class SampleProcessor(object):
    ...
    def _compute_samples_data(self, paths):
        ...
        for idx, path in enumerate(paths):
            if self.discount == 1:
                path["returns"] = utils.dummy_cumsum(path["rewards"])
            else:
                """ YOUR CODE HERE FOR PROBLEM 1A --- PROVIDED"""
                # hint: you need to implement this in hw5.utils.utils, no code here
                # Make sure you fully understand dummy_cumsum
        ...
        paths = self._compute_advantages(paths, all_path_baselines, self.use_gae)
    ...
    def _compute_advantages(self, paths, all_path_baselines, use_gae=False):
        ...
        if use_gae:
            ...
            for idx, path in enumerate(paths):
                ...
                path_returns = np.append(path['returns'], 0)
                path["advantages"] = path["rewards"] + self.discount * path_returns[1:] - path_baselines[:-1]
        return paths
